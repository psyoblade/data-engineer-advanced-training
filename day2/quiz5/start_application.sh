...
docker run --rm --log-driver=fluentd --log-opt tag=docker.{{.DaemonName}} ubuntu echo '{"message":"send message with daemon-name"}'
docker run --rm --log-driver=fluentd --log-opt tag=docker.{{.ImageFullID}} ubuntu echo '{"message":"send message with image-full-id"}'
